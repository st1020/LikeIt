  ...
  initSearch() {
    ...
    const autosearch = autocomplete("#search-input-".concat(suffix), {
      ...
      source: (query, callback) => {
            ...
            matches.forEach(_ref2 => {
              ...
              if (key == "title" || key == "content") {
                ...
                indices.forEach(region => {
                  ...
                  text += value.substring(offset, region[0]) + "<".concat(highlightTag, ">") + value.substring(region[0], temp) + "</".concat(highlightTag, ">");
                  offset = temp;
                });
        ...
        if (!this._index) {
          fetch(searchConfig.indexURL).then(response => response.json()).then(data => {
